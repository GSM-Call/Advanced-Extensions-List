...
*/
	...
	*/
	...
	function SIPpeerstatus() {
		$this->response_catch = array();
		$this->add_event_handler("peerstatus", array($this, 'Mpeers_catch'));
		$this->add_event_handler("sippeerstatuscomplete", array($this, 'Mpeers_catch'));
		$response = $this->send_request('SIPpeerstatus');
		if ($response["Response"] == "Success") {
			$this->wait_response(true);
			stream_set_timeout($this->socket, 90);
		} else {
			return false;
		}
		$res = $this->response_catch;
		unset($this->response_catch);
		// Asterisk 12 can sometimes dump extra garbage after the
		// output of this. So grab it, and discard it, if it's
		// pending.
		// Note that this has been reported as a bug and should
		...
		while (fgets($this->socket)) { /* do nothing */ }
		stream_set_blocking($this->socket, true);
		unset($this->event_handlers['sippeerstatus']);
		unset($this->event_handlers['sippeerstatuscomplete']);
		return $res;
	}

	private function Mpeers_catch($event, $data, $server, $port) {
		switch($event) {
			case 'sippeerstatuscomplete':
				stream_set_timeout($this->socket, 0, 1);
				break;
			default:
				$this->response_catch[] =  $data;
	...
	*/
 	public function SIPpeers() {
		$this->response_catch = array();
		$this->add_event_handler("peerentry", array($this, 'Mmpeers_catch'));
		$this->add_event_handler("peerlistcomplete", array($this, 'Mmpeers_catch'));
		$response = $this->send_request('SIPpeers');
		if ($response["Response"] == "Success") {
			$this->wait_response(true);
			stream_set_timeout($this->socket, 90);
		} else {
		...
		while (fgets($this->socket)) { /* do nothing */ }
		stream_set_blocking($this->socket, true);
		unset($this->event_handlers['peerentry']);
		unset($this->event_handlers['peerlistcomplete']);
		return $res;

	}

	private function Mmpeers_catch($event, $data, $server, $port) {
		switch($event) {
	...
	public function SIPshowpeer($dev) {
		$params = array("peer" => $dev);
		$response = $this->send_request('SIPshowpeer', $params);
		return $response;
		}

	function mSIP() {
		$r = $this->command("sip show peers");
		print_r($r);
		// $data = explode("\n",$r["data"]);
		// array_shift($data);
		// print_r($data);
	...
	function PJSIPShowAors() {
		$this->response_catch = array();
		$this->add_event_handler("aorlist", array($this, 'PJSIPaors_catch'));
		$this->add_event_handler("aorlistcomplete", array($this, 'PJSIPaors_catch'));
		$response = $this->send_request('PJSIPShowAors');
		if ($response["Response"] == "Success") {
			$this->wait_response(true);
			stream_set_timeout($this->socket, 90);
		} else {
		...
		while (fgets($this->socket)) { /* do nothing */ }
		stream_set_blocking($this->socket, true);
		unset($this->event_handlers['aorlist']);
		unset($this->event_handlers['aorlistcomplete']);
		return $res;
	}

	private function PJSIPaors_catch($event, $data, $server, $port) {
		switch($event) {
	...
	function PJSIPShowContacts() {
		$this->response_catch = array();
		$this->add_event_handler("contactlist", array($this, 'PJSIPShowContacts_catch'));
		$this->add_event_handler("contactlistcomplete", array($this, 'PJSIPShowContacts_catch'));
		$response = $this->send_request('PJSIPShowContacts');
		if ($response["Response"] == "Success") {
			$this->wait_response(true);
			stream_set_timeout($this->socket, 90);
		} else {
		...
		while (fgets($this->socket)) { /* do nothing */ }
		stream_set_blocking($this->socket, true);
		unset($this->event_handlers['contactlist']);
		unset($this->event_handlers['contactlistcomplete']);
		return $res;
	}

	private function PJSIPShowContacts_catch($event, $data, $server, $port) {
		switch($event) {
	...
	function PJSIPShowRegistrationsOutbound() {
		$this->response_catch = array();
		//$this->add_event_handler("*", array($this, 'PJSIPShowRegistrationsOutbound_catch'));
		$this->add_event_handler("OutboundRegistrationDetail", array($this, 'PJSIPShowRegistrationsOutbound_catch'));
		$this->add_event_handler("AuthDetail", array($this, 'PJSIPShowRegistrationsOutbound_catch'));
		$this->add_event_handler("OutboundRegistrationDetailComplete", array($this, 'PJSIPShowRegistrationsOutbound_catch'));
		$response = $this->send_request('PJSIPShowRegistrationsOutbound');
		if ($response["Response"] == "Success") {
			$this->wait_response(true);
			stream_set_timeout($this->socket, 90);
		} else {
		...
		while (fgets($this->socket)) { /* do nothing */ }
		stream_set_blocking($this->socket, true);
		unset($this->event_handlers['OutboundRegistrationDetail']);
		unset($this->event_handlers['AuthDetail']);
		unset($this->event_handlers['OutboundRegistrationDetailComplete']);
		return $res;
	}

	private function PJSIPShowRegistrationsOutbound_catch($event, $data, $server, $port) {
		switch($event) {
			case 'OutboundRegistrationDetailComplete':
				stream_set_timeout($this->socket, 0, 1);
				break;
			default:
				$this->response_catch[] =  $data;
